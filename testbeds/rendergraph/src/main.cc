...
constexpr unsigned FRAMES_IN_FLIGHT = 3;
...
int main(int argc, char *argv[])
{
    ...
    vkfw::init();
    auto window = vkfw::createWindowUnique(letc::WindowBuilder{});
    auto instance = std::make_unique<letc::Instance>(letc::InstanceBuilder{}.setDebug(true));
    VULKAN_HPP_DEFAULT_DISPATCHER.init(instance->instance);
    auto surface = vkfw::createWindowSurfaceUnique(*instance, *window);

    auto db = letc::DeviceBuilder{};
    db.requestQueue("graphics", vk::QueueFlagBits::eGraphics);
    auto device = std::make_unique<letc::Device>(*instance, db);
    auto allocator = std::make_unique<letc::Allocator>(*instance, *device);
    auto swapchain = std::make_unique<letc::Swapchain>(*window, *surface, *device);
    auto gQueue = device->queues["graphics"].queue;
    auto gQueueIndex = device->queues["graphics"].queueFamilyIndex;

    auto commandPoolInfo = vk::CommandPoolCreateInfo{};
    commandPoolInfo.setQueueFamilyIndex(gQueueIndex);
    commandPoolInfo.setFlags(vk::CommandPoolCreateFlagBits::eResetCommandBuffer);
    auto commandPool = device->device.createCommandPoolUnique(commandPoolInfo);
    auto commandBufferInfo = vk::CommandBufferAllocateInfo{};
    commandBufferInfo.setCommandBufferCount(3);
    commandBufferInfo.setCommandPool(*commandPool);
    commandBufferInfo.setLevel(vk::CommandBufferLevel::ePrimary);
    auto commandBuffers = device->device.allocateCommandBuffersUnique(commandBufferInfo);
}
...
//             *allocator, sizeof(GlobalUniforms), vk::BufferUsageFlagBits::eUniformBuffer,
...
//             allocator->allocator, static_cast<uint32_t>(window->getWidth()),
//             static_cast<uint32_t>(window->getHeight()), vk::Format::eD32Sfloat, std::vector<float>(window->getWidth()
//             * window->getHeight(), 0.0f), vk::ImageUsageFlagBits::eDepthStencilAttachment,
...
//         pointsRenderer->material->updateDescriptorBufferInfo(0, 0, *camera->buffer, 0,
//         sizeof(letc::Camera::Uniform)); pointsRenderer->material->updateDescriptorSet(0);
...
//                                        &depthBarrier);
//         commandBuffer->pipelineBarrier(vk::PipelineStageFlagBits::eTopOfPipe,
//                                        vk::PipelineStageFlagBits::eColorAttachmentOutput, {}, 0, nullptr, 0, nullptr,
//                                        1, &colorBarrier);
